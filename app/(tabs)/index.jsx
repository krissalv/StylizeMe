...
useEffect(() => {
  const loadNews = async () => {
    const news = await fetchFashionSustainabilityNews();
    setArticles(news);
    setLoading(false);
  };
  loadNews();
  ...
  const fetchFashionSustainabilityNews = async () => {
    try{
      const response = await axios.get(BASE_URL,{
        params: {
          q: 'fashion sustainability',
          language: 'en',
          sortBy: 'publishedAt',
          pageSize: 5,
          apiKey: API_KEY,
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <ScrollView contentContainerStyle={styles.scrollContent}>
          ...
          <View style = {styles.userContainer}>
          <Text style={styles.sectionTitle}>Fashion Sustainability News</Text>
            {newsLoading ? (
              ...
              newsArticles.map((article, index) => (
                <View key={index} style={styles.newsCard}>
                  ...
                  <Text numberOfLines={3} style={styles.newsDescription}>{article.description}</Text>
                </View>
              ))
            ) : (
